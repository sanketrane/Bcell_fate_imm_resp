functions{
  ...
  real CAR_positive_FOB(real time){
   ...
   real[] ODE(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
     ...
     real delta_GC = delta * (1 + exp(-nu * (time - tau)^2));
     ...
     dydt[2] = (alpha2/(y[2] + y[3])) * CAR_positive_FOB(time) + mu * y[3] - delta_GC * y[2];
     dydt[3] = (alpha2/(y[2] + y[3])) * CAR_negative_FOB(time) - mu * y[3] - delta_GC * y[3];
