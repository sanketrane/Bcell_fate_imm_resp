functions{
  ...
  real CAR_positive_FOB(real time){
   ...
   real[] ODE(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
    ...
    real alpha_t_MZ = alpha0 * (1 + time * exp(-nu * time));

     // the system of ODEs
     real dydt[3];

     dydt[1] = (alpha_t_MZ/M) * F - lambda * y[1];

     dydt[2] = (alpha_t_MZ/y[3]) * (CAR_positive_FOB(time) - F * y[2]) + mu * (1 - y[2]);
     dydt[3] = alpha_t_MZ * F - delta * y[3];
...
model{
  ...
  alpha0 ~ normal(0.01, 2);
  nu ~ normal(0.1, 2);
  lambda ~ normal(0.05, 2);
  mu ~ normal(0.1, 2);
  delta ~ normal(0.1, 2);
