functions{
  ...
  real CAR_positive_FOB(real time){
   ...
   real[] ODE(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
     ...
     //real alpha_GC = alpha2/(1 + exp(-nu * (time - tau)^2));
     real alpha_GC = alpha2 * exp(nu *time)/(1 +  exp(nu * (time - tau)));

     // the system of ODEs
     real dydt[3];

     dydt[1] = (alpha1/M) * y[2] - lambda * y[1];

     dydt[2] = alpha_GC  * CAR_positive_FOB(time)  - delta * y[2];
     dydt[3] = alpha2 * CAR_negative_FOB(time)- delta * y[3];
