functions{
  ...
  real CAR_positive_FOB(real time){
   ...
   real[] ODE(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
    ...
    real delta_GC = delta/(1 + ((y[2] + y[3])/size_dens)^2);

     // the system of ODEs
     real dydt[3];

     dydt[1] = (alpha1/M) * F - lambda * y[1];

     dydt[2] = alpha2 * CAR_positive_FOB(time) + mu * y[3]  + delta_GC * y[2];
     dydt[3] = alpha2 * CAR_negative_FOB(time) - mu * y[3] + delta_GC * y[3];
...
transformed parameters{
  ...
  real G0 = exp(11.3);//exp(G0_Log);      // transformed parameters for better/faster sampling
  real size_dens = exp(size_dens_Log);      // transformed parameters for better/faster sampling
...
model{
  ...
  alpha1 ~ normal(0.01, 2);
  lambda ~ normal(0.05, 0.5);
  mu ~ normal(0.1, 2);
  delta ~ normal(0.05, 0.5);
  alpha2 ~ normal(0.01, 2);


  //G0_Log ~ normal(11, 2.5);
  size_dens_Log ~ normal(11, 2.5);

  fM_0 ~ normal(0.01, 0.2);
  fG_0 ~ normal(0.3, 0.2);
