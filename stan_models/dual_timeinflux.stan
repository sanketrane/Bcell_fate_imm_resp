...
generated quantities{
  ...
  real FOtoCARMZ_pred[numPred]; real MZtoCARMZ_pred[numPred]; real FOtoGCPred[numPred];
  ...
  for (i in 1:numPred){
    ...
    // Influx into CAR MZ
    FOtoCARMZ_pred[i] = alpha1 * (CAR_positive_FOB(ts_pred[i]) + exp(17.15))/(y1_mean_pred[i]);
