functions{
  ...
  real CAR_positive_FOB(real time){
   ...
   real[] ODE_sys(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
     ...
     dydt[2] = mu_tau * Total_FoB(time) + beta_tau * CAR_negative_MZB(time) - lambda_WT * y[2];
...
generated quantities{
   ...
   real beta_pred[numPred]; real mu_pred[numPred]; real alpha_pred[numPred];
   ...
   for (i in 1:numPred){
     ...
     beta_pred[i] = beta/(1 + exp(nu1 *(ts_pred[i] - 4.0)^2));
