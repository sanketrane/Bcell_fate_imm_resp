  ...
  real CAR_positive_FOB(real time){
   ...
   real[] ODE_sys(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
    ...
    dydt[3] = y[2] * alpha - alpha * y[3];
     ...
     dydt[4] = alpha * y[3] - delta * y[3];
     // CAR positive MZB cells in WT
     dydt[5] = Beta * CAR_negative_MZB(time) - lambda_WT * y[5];
     // CAR positive MZB cells in N2KO
     dydt[6] = Beta * CAR_negative_MZB(time) - lambda_N2KO * y[6];
     return dydt;
   }
...
transformed parameters{
  ...
  real CAR_GC0 = exp(10.1);              // transformed parameters for better/faster sampling
  real CAR_MZ0 = exp(9.8);              // transformed parameters for better/faster sampling
  real CAR_MZ0N2k0 = exp(8.03);          // transformed parameters for better/faster sampling
